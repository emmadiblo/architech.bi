...
var swipeAction = function(container,opt,vertical) {	
	...
	var notonbody =  _ISM || _R.get_browser()==="Firefox",
		...
		swipe_wadir = opt.navigation.touch.swipe_direction || "horizontal";

	pagescroll = vertical == "swipebased" && swipe_wadir=="vertical" ? "none" : vertical ? "vertical" : pagescroll;
		...
		swipeStatus:function(event,phase,direction,distance,duration,fingerCount,fingerData) {			
			...
			if ((
				...
				if (phase=="end" && !istt) {		
					...
					if ((swipe_wadir=="horizontal" && direction == "left") || (swipe_wadir=="vertical" && direction == "up")) {
						opt.sc_indicator_dir = 0;
						_R.callingNewSlide(opt,opt.c,1);
						return false;
					}
					if ((swipe_wadir=="horizontal" && direction == "right") || (swipe_wadir=="vertical" && direction == "down")) {
						opt.sc_indicator_dir = 1;
						_R.callingNewSlide(opt,opt.c,-1);	
						return false;
					}
